...
class LeakTest_RecycleAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
...
//        }
        ...
        holder.itemView.setOnClickListener {
            ...
            if (!gasRoomInfo[tmpPositon].leakTest) {
                it.setBackgroundColor(Color.parseColor("#ff9800"))
                gasRoomInfo[tmpPositon].leakTest = true
            }else{
                it.setBackgroundColor(Color.parseColor("#ffffff"))
                gasRoomInfo[tmpPositon].leakTest = false
            }

        }
    }

    fun initView(holder: RecyclerView.ViewHolder){
        for (i in gasRoomInfo){
            if (!i.leakTest){
                holder.itemView.setBackgroundColor(Color.parseColor("#ffffff"))
            }else{
                holder.itemView.setBackgroundColor(Color.parseColor("#ff9800"))
