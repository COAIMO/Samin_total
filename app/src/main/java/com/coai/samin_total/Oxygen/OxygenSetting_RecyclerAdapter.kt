...
class OxygenSetting_RecyclerAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    var oxygenInfo = mutableListOf<SetOxygenViewData>()

    private var oldPosition = -1
    private var selectedPosition = -1

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return OxygenSettingViewHodler(
    ...
    override fun getItemCount(): Int {
        return oxygenInfo.size
    }


    fun submitList(viewData: MutableList<SetOxygenViewData>) {
        this.oxygenInfo = viewData
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        (holder as OxygenSettingViewHodler).bind(oxygenInfo[position])
    ...
    inner class OxygenSettingViewHodler(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(oxygenInfo: SetOxygenViewData) {
            modelNum.text = oxygenInfo.model.toString()
            idNum.text = oxygenInfo.id.toString()
            sensorNum.text = oxygenInfo.port.toString()
        }
    }
}